...
const fetchTrainsList = async () => {
  try {
    if (selectedStationId.value) {
      ...
      const routeResponse = await fetch('http://ec2-34-238-252-143.compute-1.amazonaws.com/api/train-routes');
      ...
      if (filteredRoutes.length > 0) {
        ...
        const scheduleResponse = await fetch('http://ec2-34-238-252-143.compute-1.amazonaws.com/api/schedules');
        ...
        const trainResponse = await fetch('http://ec2-34-238-252-143.compute-1.amazonaws.com/api/trains');
...
async function fetchTrainLocation2(train_id) {
  try {
    if (train_id == null) {
      ...
      const response = await fetch('http://ec2-34-238-252-143.compute-1.amazonaws.com/api/real-location/'+train_id);
...
const fetchTrains = async () => {
  try {
    const response = await fetch('http://ec2-34-238-252-143.compute-1.amazonaws.com/api/trains');
    const data = await response.json();
    trains.value = data;
  } catch (error) {
...
const fetchStations = async () => {
  try {
    const response = await fetch('http://ec2-34-238-252-143.compute-1.amazonaws.com/api/stations');
    const data = await response.json();
    stations.value = data;
  } catch (error) {
...
const fetchTrainLocation = async () => {
  try {
    ...
    if (selectedTrainId.value == null) {
      ...
      const response = await fetch('http://ec2-34-238-252-143.compute-1.amazonaws.com/api/real-location/'+selectedTrainId.value);
      const data = await response.json();
      if (data && data.longitude && data.latitude) {
        const { longitude, latitude } = data;
        if (!map.value) {
